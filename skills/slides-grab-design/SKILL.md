...
3. Run `npm exec -- slides-grab validate --slides-dir <path>` after generation or edits.
4. If validation fails, automatically fix the source slide HTML/CSS and re-run validation until it passes.
5. Run `npm exec -- slides-grab build-viewer --slides-dir <path>` only after validation passes.
...
- Do not present slides for review until `npm exec -- slides-grab validate --slides-dir <path>` passes.
