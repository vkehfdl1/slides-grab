...
const EDITOR_PPT_DESIGN_SKILL_FALLBACK = [
  ...
  '2. When a slide needs bespoke imagery, prefer `slides-grab image --prompt "<prompt>" --slides-dir <path>` so Codex/OpenAI saves a local asset under `<slides-dir>/assets/`.',
  ...
  '- Prefer `slides-grab image` with Codex/OpenAI by default for bespoke imagery when it improves the slide.',
...
const DETAILED_DESIGN_SKILL_FALLBACK = [
  ...
  '- Use `slides-grab image --prompt "<prompt>" --slides-dir <path>` with Codex/OpenAI by default when the slide needs bespoke imagery.',
