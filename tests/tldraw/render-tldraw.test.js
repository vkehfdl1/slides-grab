...
test('normalizeTldrawSnapshot accepts tldraw file records and store snapshots', () => {
  const fromRecords = normalizeTldrawSnapshot({
    schema: { schemaVersion: 2, sequences: {} },
    records: [
      { id: 'shape:one', typeName: 'shape', x: 10 },
      { id: 'page:one', typeName: 'page' },
    ],
  });
  ...
  assert.deepEqual(fromRecords, {
    schema: { schemaVersion: 2, sequences: {} },
    store: {
      'shape:one': { id: 'shape:one', typeName: 'shape', x: 10 },
